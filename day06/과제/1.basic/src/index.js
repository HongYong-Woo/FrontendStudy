...
const completeFolderTree = function (folderList) {
    for (let i = 0; i < folderList.length; i++) {
        folderList[i].addEventListener('click', (event) => {
            // 여기에서 작업하세요.
            const Folders = event.target.parentNode;
            Folders.classList.toggle('isOpen');
      
            
            if(Folders.classList.contains('isOpen')) {
                Folders.querySelector('.lower-folder').style.display = 'block';
            }
            else {
                Folders.querySelector('.lower-folder').style.display = 'none';
            }
            
            if(!folderList[0].parentNode.classList.contains('isOpen')) {
                lowerFolder.forEach((n) => {
                    n.style.display = 'none';
                    if(n.parentNode.classList.contains('isOpen')) {
                        n.parentNode.classList.remove('isOpen')
                    }
                });
            ...
            // console.log(event.target.parentNode.querySelector('.toggle'))
